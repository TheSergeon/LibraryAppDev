...
exports.formatDetectedFolders = exports.printSupportedFrameworks = exports.isDescendantPath = exports.detectFrameworks = exports.detectProjectFolders = exports.generateConfiguration = void 0;
...
async function generateConfiguration(app, api) {
    let config = {
        appLocation: config_1.DEFAULT_CONFIG.appLocation,
        outputLocation: config_1.DEFAULT_CONFIG.outputLocation,
    };
    if (!app && !api) {
        utils_1.logger.silly("No known frameworks detected");
        return config;
    }
    let name = "";
    if (app) {
        ...
        config.appLocation = (0, utils_1.removeTrailingPathSep)(config.appLocation);
        config.outputLocation = await computePath(config.appLocation, config.outputLocation);
        config.outputLocation = path_1.default.normalize(path_1.default.relative(config.appLocation, config.outputLocation));
        config.outputLocation = (0, utils_1.removeTrailingPathSep)(config.outputLocation);
    }
    if (api) {
        ...
        if (computedApiLocation !== api.rootPath) {
            ...
            utils_1.logger.silly(`Built API location "${computedApiLocation}" does not match root API location ${api.rootPath}, which is not supported yet`);
        }
        config.apiLocation = (0, utils_1.removeTrailingPathSep)(api.rootPath);
    }
    const appRootPath = app && (0, utils_1.removeTrailingPathSep)(app.rootPath);
    if (appRootPath && config.appBuildCommand && appRootPath !== config.appLocation) {
        ...
        let commandPath = path_1.default.relative(config.appLocation, appRootPath);
        commandPath = (0, utils_1.hasSpaces)(commandPath) ? `"${commandPath}"` : commandPath;
        config.appBuildCommand = `cd ${commandPath} && ${config.appBuildCommand}`;
    }
    const apiRootPath = api && (0, utils_1.removeTrailingPathSep)(api.rootPath);
    if (apiRootPath && config.apiBuildCommand && apiRootPath !== config.apiLocation) {
        ...
        // we need to adjust the build command to run in the correct path.
        let commandPath = path_1.default.relative(config.apiLocation, apiRootPath);
        commandPath = (0, utils_1.hasSpaces)(commandPath) ? `"${commandPath}"` : commandPath;
...
async function computePath(basePath, additionalPath) {
    ...
    if (!additionalPath.startsWith("{")) {
        return path_1.default.join(basePath, additionalPath);
    }
    ...
    const json = await (0, utils_1.safeReadJson)(file);
    if (!json) {
    ...
    try {
        ...
        if (result) {
            return path_1.default.join(basePath, result);
        }
    }
    catch (error) {
        const err = error;
        utils_1.logger.silly(err.stack || err.message);
        throw new Error(`Invalid expression "${expression}" in dynamic path: ${additionalPath}`);
    }
    return basePath;
}
async function detectProjectFolders(projectPath = ".") {
...
exports.detectProjectFolders = detectProjectFolders;
async function detectFrameworks(projectFiles, frameworks) {
    ...
    for (const framework of frameworks) {
        ...
        if (framework.parent) {
            ...
            files.push(...((_a = parent.files) !== null && _a !== void 0 ? _a : []), ...(parent.packages ? [packageJsonFile] : []));
        }
        files.push(...((_b = framework.files) !== null && _b !== void 0 ? _b : []), ...(framework.packages ? [packageJsonFile] : []));
...
async function detectApiFrameworks(projectFiles) {
    const detectedApiFolders = await detectFrameworks(projectFiles, frameworks_1.apiFrameworks);
    utils_1.logger.silly(formatDetectedFolders(detectedApiFolders, "api"));
    return detectedApiFolders;
}
async function detectAppFrameworks(projectFiles) {
    const detectedAppFolders = await detectFrameworks(projectFiles, frameworks_1.appFrameworks);
    utils_1.logger.silly(formatDetectedFolders(detectedAppFolders, "app"));
...
async function matchPackages(framework) {
    ...
    const rootPathsMatches = await asyncFilter(framework.rootPaths, async (rootPath) => {
        var _a, _b;
        const packageJsonPath = path_1.default.join(rootPath, packageJsonFile);
        const packageJson = await (0, utils_1.safeReadJson)(packageJsonPath);
        if (!packageJson) {
            return false;
        }
...
async function matchContains(framework, files) {
    ...
    const rootPathsMatches = await asyncFilter(framework.rootPaths, async (rootPath) => {
        ...
        return asyncEvery(Object.entries(framework.contains), async ([filename, stringToFind]) => {
            ...
            const content = await (0, utils_1.safeReadFile)(file);
            if (!content) {
                return false;
            }
...
function isDescendantPath(testedPath, referencePath) {
    return testedPath !== referencePath && !path_1.default.relative(referencePath, testedPath).startsWith("..");
}
...
function filterDescendantFolders(folders) {
    ...
    utils_1.logger.silly(`Found descendant folders to exclude:`);
    utils_1.logger.silly(`- ${Array.from(descendantPaths).join("\n- ")}`);
...
function filterPreemptedFrameworks(detectedFolders) {
    for (const folder of detectedFolders) {
        ...
        utils_1.logger.silly(`Found frameworks to override in path ${folder.rootPath}: ${Array.from(overridenFrameworkIds).join(",")}`);
        ...
        for (const framework of frameworks) {
            if (framework.parent && frameworkIndexById[framework.parent] === undefined) {
                ...
                if (parentIndex === -1) {
                    ...
                    utils_1.logger.silly(`Framework ${framework.id} has parent ${framework.parent} but it's not detected`);
...
function findAllFiles(fileglob, files) {
    const { regex } = (0, globrex_1.default)(`?(*${path_1.default.sep})${fileglob}`, { extended: true, flags: "i" });
...
function filesFromRootPath(rootPath, files) {
    if (rootPath === "." || rootPath === `.${path_1.default.sep}`) {
        return files;
    }
...
function findRootPathsForFiles(fileglobs, files) {
    ...
    const uniqueRootPaths = new Set(foundFiles[0].map((file) => path_1.default.dirname(file)));
    const otherFoundFiles = foundFiles.slice(1);
    const possibleRootPaths = [...uniqueRootPaths].filter((p) => otherFoundFiles.every((files) => files.some((file) => path_1.default.dirname(file) === p)));
...
function printSupportedFrameworks(showList = false) {
    if (showList) {
        utils_1.logger.info(`Supported api frameworks: ${frameworks_1.apiFrameworks.length}`);
        utils_1.logger.info(`- ${frameworks_1.apiFrameworks.map((f) => f.name).join("- \n")}`);
        utils_1.logger.info(`Supported app frameworks: ${frameworks_1.apiFrameworks.length}`);
        utils_1.logger.info(`- ${frameworks_1.appFrameworks.map((f) => f.name).join("- \n")}`);
    }
    else {
        utils_1.logger.info(`Supported frameworks:`);
        utils_1.logger.info(`- api: ${frameworks_1.apiFrameworks.length}`);
        utils_1.logger.info(`- app: ${frameworks_1.appFrameworks.length}`);
    }
}
exports.printSupportedFrameworks = printSupportedFrameworks;
function formatDetectedFolders(folders, type) {
...
exports.formatDetectedFolders = formatDetectedFolders;
