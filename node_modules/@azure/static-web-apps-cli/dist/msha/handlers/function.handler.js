...
async function resolveLocalhostByFetching(url) {
    const { protocol, port } = (0, net_1.parseUrl)(url);
    const fetchOneOfUrl = [`${protocol}//127.0.0.1:${port}`, `${protocol}//[::1]:${port}`];
    let promises = fetchOneOfUrl.map((Url) => {
        return (0, node_fetch_1.default)(Url)
            .then((response) => {
            if (response.ok || response.redirected) {
                core_1.logger.silly(`Fetch ${Url} successfully`);
                return Url;
            }
            else {
                core_1.logger.silly(`Fetch ${Url} failed with status ${response.status} ${response.statusText}`);
                throw new Error(`Fetch ${Url} failed with status ${response.status} ${response.statusText}`);
            }
        })
            .catch((err) => {
            core_1.logger.silly(`Could not fetch ${Url}`);
            throw err;
        });
    });
    try {
...
function handleFunctionRequest(req, res) {
    ...
    const { protocol, hostname, port } = (0, net_1.parseUrl)(cliApiUri);
    ...
    proxyApi.web(req, res, {
        target,
    }, (0, request_middleware_1.onConnectionLost)(req, res, target, "↳"));
...
async function validateFunctionTriggers(url) {
    const { hostname } = (0, net_1.parseUrl)(url);
    if (hostname === "localhost") {
        try {
            url = await resolveLocalhostByFetching(url);
        }
        catch (error) {
            core_1.logger.error(error === null || error === void 0 ? void 0 : error.message, true);
        }
    }
    try {
